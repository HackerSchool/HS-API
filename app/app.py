...
def create_app(config_class=Config, *, member_repo=None, project_repo=None, participation_repo=None, task_repo=None,
    ...
    # Use ORIGINS_WHITELIST from config, or "*" if empty/not set
    cors_origins = flask_app.config.get("ORIGINS_WHITELIST", ["*"])
    ...
    # Explicitly set cookie domain for cross-subdomain support
    # Flask-Session should read from config, but we ensure it's set
    if flask_app.config.get("SESSION_COOKIE_DOMAIN"):
        flask_app.config["SESSION_COOKIE_DOMAIN"] = flask_app.config["SESSION_COOKIE_DOMAIN"]
    ...
    if team_task_repo is None:
        team_task_repo = TeamTaskRepository(db=db)
    ...
    task_bp = create_task_bp(task_repo=task_repo, team_task_repo=team_task_repo,
                        participation_repo=participation_repo,
