...
from flask import session, abort, g, redirect, current_app
...
class AuthController:
    ...
    def login_member(self, fn):
        ...
        def wrapper(*args, **kwargs):
            ...
            if redirect_uri:
                if member is None:
                    return redirect(redirect_uri + f"?login=fail")
                session["id"] = member.id
                session.permanent = True  # Mark session as permanent to ensure cookie is set correctly
                
                # Log session configuration for debugging
                print(f"[DEBUG] Setting session for member {member.id}")
                print(f"[DEBUG] SESSION_COOKIE_DOMAIN: {current_app.config.get('SESSION_COOKIE_DOMAIN')}")
                print(f"[DEBUG] SESSION_COOKIE_SAMESITE: {current_app.config.get('SESSION_COOKIE_SAMESITE')}")
                print(f"[DEBUG] SESSION_COOKIE_SECURE: {current_app.config.get('SESSION_COOKIE_SECURE')}")
                print(f"[DEBUG] Redirect URI: {redirect_uri}")
                
                # Create redirect response
                redirect_response = redirect(redirect_uri + f"?login=success&username={member.username}")
                
                # Ensure session is saved and cookie is set with correct domain
                # Flask-Session should handle this automatically based on config
