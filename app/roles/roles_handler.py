...
class RolesHandler:
    def __init__(self):
        self.roles: dict[str : Tuple[int, Set[str]]] = (
            None  # indexes level and permissions set by role name
        )
    ...
    def has_higher_level(self, roles_list: List[str], role: str):
        ...
        return highest_lvl < self._get_role_level(
            role
        )  # < instead of <= means we cannot add "horizontally"
    ...
    def init_app(self, app: Flask) -> None:
        ...
        if roles == "":
            app.logger.error("Missing 'roles' key")
            raise KeyError("roles")

        if not isinstance(roles, list):
            app.logger.error(
                f"Invalid 'roles' key value, expected list got {type(roles)}"
            )
        ...
        for role in roles:
            if not isinstance(role, dict):
                raise ValueError(
                    f"Invalid role entry type, expected JSON object got {type(role)}"
                )
            ...
            if not isinstance(name, str):
                raise ValueError(
                    f"Invalid 'name' key value type, expected string got {type(name)}"
                )
            ...
            if not isinstance(permissions, list):
                raise ValueError(
                    f"Invalid 'permissions' key value type, expected list got {type(permissions)}"
                )
            ...
            if not isinstance(level, int):
                raise ValueError(
                    f"Invalid 'level' key value type, expected int got {type(level)}"
                )
