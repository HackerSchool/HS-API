...
class Config:
    ...
    ROOT_URI = _get_env_or_default("ROOT_URI", "http://localhost:8080")

    ENABLED_ACCESS_CONTROL: bool = _get_bool_env_or_false("ENABLED_ACCESS_CONTROL") or True
    ORIGINS_WHITELIST: List = _get_env_or_default("ORIGINS_WHITELIST", "http://localhost:3000 http://localhost:5173").split()

    CLIENT_ID:     str =           _get_env_or_default("CLIENT_ID", "test_client_id")
    CLIENT_SECRET: str =           _get_env_or_default("CLIENT_SECRET", "test_client_secret")
    FENIX_REDIRECT_ENDPOINT: str = _get_env_or_default("FENIX_REDIRECT_ENDPOINT", "/fenix-login-callback")

    SESSION_COOKIE_SAMESITE = _get_env_or_default("SAME_SITE_POLICY", "Lax")
    SESSION_COOKIE_HTTPONLY = True
    # For development with HTTP, set SESSION_COOKIE_SECURE=False in .env
    SESSION_COOKIE_SECURE   = _get_env_or_default("SESSION_COOKIE_SECURE", "False").lower() == "true"
    # For cross-subdomain cookies (e.g., api.hackerleague.app and hackerleague.app)
    # Set to .hackerleague.app to allow cookies to work across subdomains
    SESSION_COOKIE_DOMAIN = _get_env_or_default("SESSION_COOKIE_DOMAIN", None)
