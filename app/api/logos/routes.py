...
from app.logos.exceptions import (
...
def get_member_logo(username):
    ...
    try:
        ...
        return jsonify(
            {"error": f"Logo not found for user {e.resource_id}"}
        ), HTTPStatus.NOT_FOUND
    except InvalidLogoTypeError:
        # this shouldn't happen
        return jsonify({"error": "Internal"}), HTTPStatus.INTERNAL_SERVER_ERROR
...
def set_member_logo(username):
    ...
    try:
        ...
        throw_api_error(
            ...
            {"error": f"Invalid Content Type: {e.content_type}"},
        )
    except InvalidLogoTypeError:
        # this shouldn't happen
        throw_api_error(
            HTTPStatus.UNSUPPORTED_MEDIA_TYPE, {"error": "Internal server error"}
        )
...
def get_project_logo(proj_name):
    ...
    try:
        ...
        return jsonify(
            {"error": f"Logo not found for project {e.resource_id}"}
        ), HTTPStatus.NOT_FOUND
    except InvalidLogoTypeError:
        # this shouldn't happen
        return jsonify({"error": "Internal"}), HTTPStatus.INTERNAL_SERVER_ERROR
