...
def fetch_access_token(url: str) -> str | None:
    ...
    if r.status_code != 200:
        current_app.logger.info(
            f"Failed fetching access token from Fenix. Status Code {r.status_code}"
        )
        return None
    ...
    try:
        ...
        current_app.logger.info(
            f"Access token fetch response from Fenix is not JSON. {e}"
        )
        return None
...
def get_user_info(
    ...
    if r.status_code != 200:
        current_app.logger.info(
            f"Failed requesting user info from Fenix. Status Code {r.status_code}"
        )
        return False, None, None, None
